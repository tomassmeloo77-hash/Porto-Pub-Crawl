...
# neutral (single-colour) Google G
...
# neutral minimal Tripadvisor owl
TRIPA='<svg viewBox="0 0 40 24" aria-label="Tripadvisor" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="12" cy="13" r="6.4"/><circle cx="28" cy="13" r="6.4"/><circle cx="12" cy="13" r="2.3" fill="currentColor" stroke="none"/><circle cx="28" cy="13" r="2.3" fill="currentColor" stroke="none"/><path d="M14.6 5.2 L20 2.4 L25.4 5.2"/></svg>'

PANEL=r"""<aside class="panel">
  <div class="phead">
    <div class="badges" aria-label="Rated on Google and Tripadvisor">__GOOGLE__ __TRIPA__</div>
...
</aside>""".replace("__GOOGLE__",GOOGLE).replace("__TRIPA__",TRIPA)
...
:root{--void:#08070a;--pink:#ff173f;--pink-soft:#ff5468;--violet:#7a0a1f;--gold:#ffc466;--lime:#22ff7a;
  ...
  --neutral:#c9c6c7;--ease-spring:cubic-bezier(.34,1.56,.64,1);--disp:'Anton',Impact,sans-serif;--body:'Manrope',system-ui,sans-serif;--mono:'Space Mono',monospace;}
...
/* big P behind title — inset from edge, overlapping the headline */
...
.badges{display:inline-flex;align-items:center;gap:14px;color:var(--neutral);}
.badges svg{height:19px;width:auto;display:block;opacity:.92;}
.rate{display:inline-flex;align-items:center;gap:7px;font-size:12px;color:var(--muted);}
.rate .stars{font-size:12px;}.rate b{color:var(--cream);font-size:13px;}
.rate .rv{color:var(--muted-2);}
.hr{height:1px;background:var(--line);margin:18px 0;}
.price{font-family:var(--disp);font-size:48px;color:#fff;line-height:.9;}
...
.inc{font-family:var(--mono);font-size:10.5px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted-2);margin:20px 0 12px;}
...
/* ===== P size/position variants ===== */
.x1 .pbig{height:clamp(300px,44vw,560px);left:clamp(12px,2.5vw,60px);}
.x1 .left{padding-left:clamp(72px,12vw,230px);}
.x2 .pbig{height:clamp(360px,52vw,660px);left:clamp(4px,1.5vw,34px);}
.x2 .left{padding-left:clamp(84px,14vw,270px);}
.x3 .pbig{height:clamp(240px,32vw,430px);left:clamp(40px,5.5vw,100px);}
.x3 .left{padding-left:clamp(60px,10vw,180px);}
.x4 .pbig{height:clamp(330px,48vw,600px);left:clamp(20px,3.5vw,70px);opacity:.72;}
.x4 .left{padding-left:clamp(72px,12vw,220px);}
.x5 .pbig{height:clamp(420px,60vw,740px);left:clamp(6px,1.5vw,28px);}
.x5 .left{padding-left:clamp(92px,16vw,300px);}
...
<div class="tagline">D4 · P grande — tamanhos &amp; posição · cartão melhorado <b id="tl">X1</b> · preview local</div>
<div class="deck">
"""
...
"""
variants=[("x1","X1 · grande, folga à esquerda"),("x2","X2 · maior"),("x3","X3 · mais pequeno, mais afastado"),
          ("x4","X4 · grande translúcido"),("x5","X5 · gigante")]
for i,(cls,name) in enumerate(variants):
    html+=SLIDE.replace("__ACTIVE__","active" if i==0 else "").replace("__CLS__",cls).replace("__NAME__",name)
...
<nav class="switch">
  <button class="on" data-i="0">X1 Grande</button>
  <button data-i="1">X2 Maior</button>
  <button data-i="2">X3 Afastado</button>
  <button data-i="3">X4 Translúcido</button>
  <button data-i="4">X5 Gigante</button>
</nav>
<script>
...
out=base/"hero-pbig-preview.html";out.write_text(html)
